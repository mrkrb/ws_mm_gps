...
#define MMGPS_NUMBER_OF_BECONS 5
#define MMGPS_SEPARATOR "\xFFG"
#define MMGPS_SEP_LEN 2
#define MMGPS_MAX_PACKET_LENGTH MMGPS_SEP_LEN + 4 + MMGPS_NUMBER_OF_BECONS * 8 + 2 + MMGPS_SEP_LEN

typedef char (*mm_gps_char_getter)(void *data);

typedef union {
  char b[MMGPS_MAX_PACKET_LENGTH];
  struct __attribute__((packed)) { // per assicurare che la struttura nella union sia in una zona continua di memoria
    uint8_t  address;
    uint8_t  type;
    uint8_t  code;
    uint8_t  code_h;
...
uint16_t mm_gps_next_raw_packet(mm_gps* gps); // restituisce n byte letti
